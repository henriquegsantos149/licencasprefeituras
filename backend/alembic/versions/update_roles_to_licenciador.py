...
def upgrade() -> None:
    # First, update existing GESTOR users to LICENCIADOR (as text)
    op.execute(text("""
        UPDATE users 
        SET role = 'licenciador' 
        WHERE role::text = 'gestor'
    """))
    ...
    op.execute(text("""
        ...
        USING CASE 
            WHEN role::text = 'empreendedor' THEN 'empreendedor'::userrole_new
            WHEN role::text = 'licenciador' THEN 'licenciador'::userrole_new
...
def downgrade() -> None:
    # Revert LICENCIADOR back to GESTOR (as text first)
    op.execute(text("""
        UPDATE users 
        SET role = 'gestor' 
        WHERE role::text = 'licenciador'
    """))
    ...
    op.execute(text("""
        ...
        USING CASE 
            ...
            WHEN role::text = 'gestor' THEN 'gestor'::userrole_old
